...
StatsDClient.prototype.formatTags = function formatTags(metric_tags) {
    ...
    if (this.options.tagFormat && this.options.tagFormat === 'graphite') {
        // Example: disk.used;datacenter=dc1;rack=a1;server=web01
        return ';' + Object.keys(tags).map(function(key) {
            return key + '=' + tags[key];
        }).join(';');
    } else if (this.options.tagFormat && this.options.tagFormat === 'telegraf') {
        return ',' + Object.keys(tags).map(function(key) {
            return key + '=' + tags[key];
        }).join(':');
    }
